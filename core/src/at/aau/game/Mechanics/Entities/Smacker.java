...
public class Smacker extends MoveableObject {
	...
	public Smacker(Vector2 position, World world) {
		...
		this.smackingAnim = world.gameplayScreen.parentGame.getAnimator().loadAnimation("gameplay/smacker-anim.png",
				0.02f, (int) SIZE.x, (int) SIZE.y);
	...
	public void render(float delta, SpriteBatch spriteBatch) {
		...
		// System.out.println(unprojected.x + " " + unprojected.y);
		// Vector2 touchPixelCoords = PixieSmack.worldToPixel(new
		// Vector2(unprojected.x, unprojected.y));
		// Vector2 touchPixelCoords = new Vector2(Gdx.input.getX(),
		// Gdx.input.getY());
		...
		spriteBatch.draw(frame.getTexture(), position.x - SMACK_OFFSET.x, position.y - SMACK_OFFSET.y, 0, 0, SIZE.x,
				SIZE.y, 1f, 1f, -45, frame.getRegionX(), frame.getRegionY(), frame.getRegionWidth(),
